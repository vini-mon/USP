...
#pragma omp declare reduction(max_index : struct max_reduction : omp_out = omp_in.max > omp_out.max ? omp_in : omp_out)
...
int summary_statistics(int*** arr, int R, int C, int A)
{
    ...
    */
    ...
    #pragma omp parallel for private(i,j,k) shared(arr, cidade_freq) collapse(3)
    for (i = 0; i < R; i++)
    {
        for (j = 0; j < C; j++)
        {
    ...
    #pragma omp parallel for private(i,j,k) shared(cidade_freq, regiao_freq)
    for (i = 0; i < R; i++)
    {
        for (j = 0; j < C; j++)
        {
    ...
    #pragma omp parallel for private(i,j) reduction(+:pais_freq[:101]) 
    for (i = 0; i < R; i++)
    {
        for (j = 0; j < 101; j++) // País
        {
            pais_freq[j] += regiao_freq[i][j];
        }
    }
    
    // Sumário estatístico em nível de cidade

    #pragma omp parallel for private(i,j) shared(cidade_stats, cidade_freq)
    for (i = 0; i < R; i++)
    {
        for (j = 0; j < C; j++)
        {
    ...
    #pragma omp parallel for private(i) shared(regiao_stats, regiao_freq)     
    for (i = 0; i < R; i++)
    {
        // Valor mínimo
    ...
    struct max_reduction melhor_regiao;
    melhor_regiao.max = 0;
    melhor_regiao.regiao = 0;

    #pragma omp parallel for private(i) reduction(max_index:melhor_regiao)
    for (i = 0; i < R; i++)
    {
        if (regiao_stats[i][3] > melhor_regiao.max)
        {
            melhor_regiao.max = regiao_stats[i][3];
            melhor_regiao.regiao = i;
        }
    }

    // Melhor cidade

    struct max_reduction melhor_cidade;
    melhor_cidade.max = 0;
    melhor_cidade.regiao = 0;
    melhor_cidade.cidade = 0;

    #pragma omp parallel for private(i,j) reduction(max_index:melhor_cidade)
    for (i = 0; i < R; i++)
    {
        for (j = 0; j < C; j++)
        {
            if (cidade_stats[i][j][3] > melhor_cidade.max)
            {
                melhor_cidade.max = cidade_stats[i][j][3];
...
int main(int argc, char *argv[])
{
    ...
    // Definir número de threads de acordo com núcleos disponíveis
